...
function extendConfig (config, options) {
  ...
  const babel = config.module.rules.find(r => r.test + '' === '/\\.jsx?$/i')
...
function normalizeComponents (components) {
  return components.map((name) => ({
    ...
    js: tryResolve(`framework7/components/${name}/${name}.js`),
    less: tryResolve(`framework7/components/${name}/${name}.less`)
  }))
}
...
const defaults = {
  ...
  app: {
    params: {
      id: 'id.nuxt7.application',
      name: 'nuxt7-application',
      theme: 'auto'
    }
  },
  view: {
